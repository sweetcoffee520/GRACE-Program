function [GRACE_all]=CS2EWH(Data_type,DeltaGC,DeltaGS,Data_number,Nlmx,De_filter,Filter_index,Lmax,De_P,De_M,r_guassian,r1_fan,r2_fan,fir,n_c,n_f,nceta,nfir)
...
if isequal(Filter_index{1},'1')
    [grace_non]=grace2ewh(Lmax,k,Nlmx,DeltaGC,DeltaGS,fir,n_c,n_f,nceta,nfir);
...
if isequal(Filter_index{2},'1')
    ...
    [grace_gau]=grace2ewh(Lmax,k,Nlmx,DeltaGC_g,DeltaGS_g,fir,n_c,n_f,nceta,nfir);
...
if isequal(Filter_index{3},'1')
    ...
    [grace_fan]=grace2ewh(Lmax,k,Nlmx,DeltaGC_f,DeltaGS_f,fir,n_c,n_f,nceta,nfir);
    disp('Results with Fan filter done!')
    GRACE_all(:,:,3)=grace_fan;
end
%%--------------------------------------------%%
if isequal(Filter_index{4},'1')
    [grace_p3m6_non]=grace2ewh(Lmax,k,Nlmx,DeltaGC_0,DeltaGS_0,fir,n_c,n_f,nceta,nfir);
...
if isequal(Filter_index{5},'1')
    ...
    [grace_p3m6_gau]=grace2ewh(Lmax,k,Nlmx,DeltaGC_g,DeltaGS_g,fir,n_c,n_f,nceta,nfir);
...
if isequal(Filter_index{6},'1')
    ...
    [grace_p3m6_fan]=grace2ewh(Lmax,k,Nlmx,DeltaGC_f,DeltaGS_f,fir,n_c,n_f,nceta,nfir);
